                    ...
                    <div class="dropdown float-right-xs">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
                            @foreach ($cinemas as $cinema)
                                <li><a href="{{ route('films.filter', isset($genreName) ? $cinema->slug .'/'. $genreSlug : $cinema->slug) }}">{{ $cinema->name }}</a></li>
                            @endforeach
                        </ul>
                    </div>
                </div>
                ...
                <div class="dropdown float-right">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
                        ...
                        @foreach ($genres as $genre)
                            <li><a href="{{ route('films.filter', isset($cinemaName) ? strtolower($cinemaName) .'/'. $genre->slug : $genre->slug) }}">{{ $genre->name }}</a></li>
                        @endforeach
                    </ul>
                </div>
            </div>
